# BargainAI - Complete Flask Backend
# Developer: Bobby Balyan
# GitHub: github.com/aimlwithbobbybalyan
# Improvements: rate limiting, better DB schema
...
# Max 5 analyse requests per IP per minute
# ═══════════════════════════════════════════
...
# ═══════════════════════════════════════════
# DATABASE — improved schema with more columns
# ═══════════════════════════════════════════
def init_db():
...
# ═══════════════════════════════════════════
# Init DB on startup (works on Render too)
# ═══════════════════════════════════════════
init_db()
...
def analyse():
    ...
    try:
        ...
        # Step 1: Vision analysis (per-image + merge)
        vision_result = run_vision_analysis(
            saved_paths, seller_price, usage_period, gadget_type, user_issues, device_model
        )

        # Step 2: Market research
        # Use device_model if user provided it — more accurate than AI guess
        search_name   = device_model if device_model else vision_result.get("product_name", "")
        search_method = "user_model" if device_model else "ai_detected"
        market_data   = fetch_market_prices(search_name, gadget_type)

        # Step 3: Normalized deal score
        ...
        # Confidence validation errors — user-friendly message
...
def get_analysis(analysis_id):
    conn = sqlite3.connect(DB_PATH)
    row  = conn.execute(
        "SELECT full_result FROM analyses WHERE id=?", (analysis_id,)
    ).fetchone()
    conn.close()
    if not row:
...
def stats():
    ...
    top_row   = conn.execute("""
        SELECT gadget_type, COUNT(*) c FROM analyses
        GROUP BY gadget_type ORDER BY c DESC LIMIT 1
    """).fetchone()
    conn.close()
    return jsonify({
...
if __name__ == "__main__":
    ...
    print("  Rate limit: 5 req/min per IP")
    print("=" * 50)
